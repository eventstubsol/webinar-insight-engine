...
/**
 * Comprehensive debug logging to verify past webinar integration
 */
...
export function logPastWebinarAPIAttempts(webinarId: string, attempts: string[]) {
  console.log(`[debug-logger] 🚀 PAST WEBINAR API ATTEMPTS for ${webinarId}:`);
  attempts.forEach((attempt, index) => {
    console.log(`[debug-logger]   ${index + 1}. ${attempt}`);
  });
}
